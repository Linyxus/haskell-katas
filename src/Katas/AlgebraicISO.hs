module Katas.AlgebraicISO where
...
isoUnMaybe (f, g) = (unMaybe . u . f . pure, unMaybe . v . g . pure)
  where u Nothing = f Nothing
        u x = x
        v Nothing = g Nothing
        v y = y
